...
from .chem_errors import InvalidCharacter, NoSuchAtom, MoreThanOneAdduct, BracketsNotPaired
...
class FormulaValidator(ChemicalFormula):
    ...
    def validate_formula(self) -> bool:
        if self._invalid_charachers():
            raise InvalidCharacter(f"Invalid character(s) {self._invalid_charachers()} in the formula {self.formula}")
        elif self._invalid_atoms():
            raise NoSuchAtom(f"No atom(s) {self._invalid_atoms()} in the periodic table!")
        elif not self._bracket_balance():
            raise BracketsNotPaired(f"The brackets {self.opener_brackets} {self.closer_brackets} are not balanced!")
        elif self._num_of_adducts() > 1:
            raise MoreThanOneAdduct(f"More than one adduct {self.adduct_symbols} in the formula")
        return True